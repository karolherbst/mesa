                  ...
                  struct nouveau_ws_bo **bo)
{
   ...
   if (config) {
      if (dev->chipset >= 0xc0) {
         info->tile_flags = (config->memtype & 0xff) << 8;
         info->tile_mode  = config->tile_mode;
      } else if (dev->chipset >= 0x80 || dev->chipset == 0x50) {
         info->tile_flags = (config->memtype & 0x07f) << 8 |
                            (config->memtype & 0x180) << 9;
         info->tile_mode  = config->tile_mode >> 4;
      } else {
         assert(false);
      }
   }
...
{
   assert(bo && prime_fd);
   struct nouveau_ws_drm *drm = bo_priv(bo)->drm;
   return drmPrimeHandleToFD(drm->fd, bo->handle, DRM_CLOEXEC, prime_fd);
}
